...
import type { LiveQuote, StreamConnectionStatus, StreamMessage } from "../types/marketData";

const WS_BASE = API_BASE.replace(/^http/, "ws");
const MIN_RECONNECT_DELAY_MS = 1000;
const MAX_RECONNECT_DELAY_MS = 15000;

interface UseAlpacaQuoteStreamResult {
...
/**
 * Subscribes to the backend's WebSocket relay of Alpaca's real-time
 * market-data stream (v0.1.12) for one symbol -- GET (WebSocket)
 ...
 * backend/app/streaming/alpaca_stream.py). Alpaca only, for now.
 *
 ...
 * backend's own reconnection to Alpaca. Those are two different
 * failure modes (browser<->backend vs. backend<->Alpaca) and both need
 * handling for updates to "just resume" without a manual refresh.
 */
export function useAlpacaQuoteStream(symbol: string): UseAlpacaQuoteStreamResult {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      if (cancelled) return;
      const url = `${WS_BASE}/market-data/stream?symbol=${encodeURIComponent(symbol)}&provider=alpaca`;
