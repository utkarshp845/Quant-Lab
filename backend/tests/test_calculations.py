...
1. The "graduation-test" example from the spec (Underlying $82, Long
   ...
   anchor for the whole calculation chain.
...
class TestGraduationExample:
    ...
    def test_z_score(self):
        ...
        assert z == pytest.approx(-0.138, abs=0.001)
    ...
    def test_probability_below_breakeven(self):
        ...
        assert prob == pytest.approx(0.445, abs=0.001)
...
class TestIntrinsicValueAndPayoff:
    LONG_STRIKE = 85
    SHORT_STRIKE = 77
    DEBIT = 4.39  # from the graduation example
    ...
    def test_underlying_above_both_strikes(self):
        ...
        assert calc.payoff_pl_per_contract(pl) == pytest.approx(-439.0)
...
class TestEdgeCases:
    ...
    def test_debit_can_be_negative_if_short_bid_exceeds_long_ask(self):
        ...
        debit = calc.debit_per_share(long_put_ask=1.00, short_put_bid=1.50)
