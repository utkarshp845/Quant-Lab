...
here ever opens a real connection to Alpaca.
"""
...
class _FakeHub:
    """Replaces app.streaming.hub.hub for these tests: subscribe()
    returns a queue pre-loaded with whatever frames the test wants sent,
    and unsubscribe() just records that it was called so a test can
    confirm the route cleans up after itself."""
...
def test_status_and_quote_frames_are_relayed_to_the_client(monkeypatch):
    ...
    assert fake_hub.unsubscribed and fake_hub.unsubscribed[0][0] == "TSLA"
...
def test_defaults_to_tsla_when_no_symbol_is_given(monkeypatch):
    ...
    class _RecordingHub(_FakeHub):
        async def subscribe(self, symbol: str) -> asyncio.Queue:
            ...
            return await super().subscribe(symbol)
